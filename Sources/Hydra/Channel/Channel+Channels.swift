...
public extension Channel {
	...
	/// Filter event
	///
	/// - complete: only terminal events (`error` or `finish`)
	/// - any: any event
	...
	/// Dispatch event from receiver by forwarding them until given channel completes (filter is `complete`)
	/// or dispatch any event (filter is `any`).
	/// Dispatch also complete if receiver complete by itself.
	///
	/// - Parameters:
	///   - filter: watchdog type, default is `any`
	///   - channel: target channel
	/// - Returns: channel
	public func until<C: ChannelProtocol>(filter: EventFilter = .any, channel: C) -> Channel<V,E> {
		return Channel<V,E>({ producer in
			
			// observe target channel events
			let targetDisposable = channel.subscribe({ event in
				switch filter {
				case .any: // any event of the target channel can concur to complete the receiver
					producer.complete()
				case .complete: // only terminal events target channel can concur to complete the receiver
					guard event.isFinal else { return }
					producer.complete()
				}
			})
			
			// forward any event of the receiver
			let selfDisposable = self.subscribe({ event in
				producer.send(event)
			})
			
			return DisposableBag(disposables: [targetDisposable,selfDisposable]) // to dispose all
		})
	}
	...
	public func merge(_ channels: [Channel<Value,Error>]) -> Channel<Value,Error> {
		...
		guard channels.count > 0 else {
			// invalid number of channels to merge, return the same unchanged channel.
